...
class CRM_Nihrnumbergenerator_StudyParticipantNumberGenerator {
  ...
  public static function generateNumber($study_number, $contact_id, $case_id) {
    ...
    $sql = "SELECT participation_data.nvpd_study_participant_id AS id 
            ...
            INNER JOIN civicrm_nihr_study study ON study.id = project_data.npd_study_id
            WHERE study.study_number = %1 and case_contact.contact_id = %2 AND  participation_data.nvpd_study_participant_id IS NOT NULL
            ORDER BY id DESC
            LIMIT 0, 1";
    ...
    $sequenceNrSql = "
            ...
            INNER JOIN civicrm_value_nihr_project_data project_data ON project_data.entity_id = participation_data.nvpd_project_id 
            INNER JOIN civicrm_nihr_study study ON study.id = project_data.npd_study_id
            WHERE study.study_number = %1 AND participation_data.`nvpd_study_participant_id` IS NOT NULL ";
